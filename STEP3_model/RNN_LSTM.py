...
# 转换为 Tensor
X_train = torch.tensor(X_train, dtype=torch.float32).unsqueeze(1)  # 添加额外维度作为时间步长
X_test = torch.tensor(X_test, dtype=torch.float32).unsqueeze(1)    # 添加额外维度作为时间步长
...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, dropout=Config.dropout_prob)
...
model = LSTMModel(Config.input_size, Config.hidden_size, Config.num_layers, Config.num_classes).to(device)
